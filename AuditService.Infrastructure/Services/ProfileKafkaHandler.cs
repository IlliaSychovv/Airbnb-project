...
public class ProfileKafkaHandler : IKafkaMessageHandler<AuditDto>
{
    ...
    public async Task HandleMessage(AuditDto message, CancellationToken cancellationToken)
    {
        var entity = new Audit()
        {
            UserId = message.UserId,
            CreatedAt = DateTime.Now,
            ProfileJson = JsonSerializer.Serialize(message)
        };
        
        _dbContext.Audits.Add(entity);
        await _dbContext.SaveChangesAsync(cancellationToken);
        
        _logger.LogInformation($"Kafka message received: {JsonSerializer.Serialize(entity)}");
    }
}
